...
import { programs as catalog } from '@/lib/programs';
...
export function Schedule({compact=false}:{compact?:boolean}) {
  ...
  useEffect(()=>setProgram(new URLSearchParams(window.location.search).get('program')||''),[]);
...
function BookingDialog({session,onClose,onSuccess}:{session:Row;onClose:()=>void;onSuccess:()=>void}) {
  ...
  const mustSign=waiver?.published && waiver?.requiredOnBooking && (profile?.waiverVersion !== waiver.version || profile?.waiverParticipantName !== profile?.childName?.trim() || profile?.waiverParticipantAge !== profile?.childAge);
